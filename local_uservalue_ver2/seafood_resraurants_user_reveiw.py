...
restaurantsTabelogInfo = pd.read_csv("./input/seasonal_restrant_info.csv").fillna("")
...
for store_name, url in restaurantsTabelogPageDict.items():
    ...
    review.df.to_csv("./local_uservalue_ver2/output_restaurant_review/{:s}.csv".format(store_name.replace("/","")) ,index=False)
